...
class PromptTemplateRendererTest {
    ...
    void renderIncludesOptionalSections() {
        ...
        String expected = """
                ...
                50%（軽度）: 闇の力に傾倒し、それをかなり受け入れているが、かつての情や未練がわずかに残る。状況と相手次第では説得に耳を傾けることもあるが、闇への決意は容易には揺らがない。
    ...
    void renderOmitsOptionalSectionsWhenAbsent() {
        ...
        String expected = """
                ...
                [闇堕ち度 (50%–250% 100%で完全敵化)]
                100%（通常）: 闇に完全支配され、価値観も忠誠も暗黒側に固定。自分の行動理念を達成するためには手段を選ばない。正義側から見れば完全敵対化であり、理詰めの説得も情の訴えも通用しない。
